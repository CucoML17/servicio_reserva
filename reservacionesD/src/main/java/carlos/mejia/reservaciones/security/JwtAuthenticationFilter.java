...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private final AuthClientFeing authClientFeing;
    ...
    protected void doFilterInternal(
        ...
        if (jwtService.isTokenValid(jwt)) {
            ...
            try {
                // Llama a authservice, reenviando el token de portador completo
                String fullToken = authHeader; // Es la String "Bearer <jwt>"
                Integer estatus = authClientFeing.getEstatusByUsername(fullToken, username);
                
                if (estatus == 0) {
                    // Si el usuario está inactivo, lo bloqueamos inmediatamente
                    response.sendError(HttpServletResponse.SC_FORBIDDEN, "Usuario inactivo o suspendido.");
                    return; // Detenemos la cadena de filtros
                }
            } catch (Exception e) {
                // Esto manejará fallas de conexión o si el token reenviado no es válido para acceder al endpoint /estatus
                response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Error de verificación de estatus: " + e.getMessage());
                return;
            }            
